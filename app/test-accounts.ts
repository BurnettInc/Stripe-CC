...
async function main(): Promise<void> {
  ...
  check("f2: install/start with cookie → 302 to marketplace authorize", withCookie.status === 302 && (withCookie.headers.get("location") || "").startsWith("https://marketplace.stripe.com/oauth/v2/authorize?"), `status=${withCookie.status}`);
